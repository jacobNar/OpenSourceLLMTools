...
def chunk_xml_bill(document_path, ollama_embeddings_model, max_chunk_size=2048):
    ...
    def process_element(element, parent_id=None, skip_children=False):
        ...
        tag_id = attributes.get('id', 'N/A')  # Get the id attribute, if it exists
        ...
        if element_length > max_chunk_size:
            ...
            if children:
                for child in children:
                    process_element(child, parent_id, skip_children=False)  # Keep the same parent_id
            else:
...
persist_dir = "./chroma_db_xml"  # Your chosen directory name
...
if not os.path.exists(persist_dir):
    ...
    documents = chunk_xml_bill('./rag-docs/BILLS-119hr4544ih.xml', ollama_emb, max_chunk_size=2048)
    print(documents)
